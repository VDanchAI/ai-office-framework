# Руководство по развёртыванию — как собрать своего персонажа

Это руководство проведёт вас через развёртывание одного персонажа или целого ИИ-офиса с использованием фреймворка из этого репозитория. Следуйте шагам по порядку. При первом прохождении ничего не пропускайте.
...
- **Аккаунт Claude Pro или Team** — необходим для Claude Projects (механизм хостинга каждого персонажа)
- **Экспертиза в вашей предметной области** — персонаж усиливает то, что вы уже знаете; он не может заменить знания, которых у вас нет
- **Чёткая роль и границы** — понимайте, что этот персонаж делает, и, не менее важно, что он не делает

Если роль ещё не ясна, сначала прочитайте `docs/architecture.md`.

---

## 2. Пошаговая инструкция: сборка одного персонажа

### Шаг 1: Определите роль

Начните отсюда, а не с контента. Персонаж с размытой ролью будет дрейфовать и давать сбои.

1. Напишите **описание роли из 2-3 предложений** — кто этот персонаж, какой домен покрывает
2. Сформулируйте одну **главную директиву** — единственную важнейшую цель, ради которой существует этот персонаж
3. Перечислите **что персонаж ДЕЛАЕТ** (5-10 конкретных действий)
4. Перечислите **что персонаж НЕ делает** (не менее 3 явных исключений)
...
KB — это место, где живёт экспертиза персонажа. Структура важна не меньше, чем контент.

1. Определите **5-10 ключевых тем**, которые персонажу нужно знать для работы
...
**Предупреждение:** KB без роутера заставляет персонажа загружать всё при каждом запросе. Это расходует токены и снижает качество ответов. Всегда включайте роутер.

---

### Шаг 3: Напишите инструкции

Файл инструкций определяет личность персонажа, его поведение и ограничения. Именно это вставляется в поле Custom Instructions в Claude.
...
|---|---|---|
| **Личность** | Кто такой персонаж, его голос и роль | Дрейф роли — персонаж пытается быть всем сразу |
| **Область действия** | Что персонаж обрабатывает, а что передаёт другим | Расширение области — персонаж делает работу, которую не должен |
...
**Всегда включайте секцию "Кем я НЕ являюсь".** Перечислите 3-5 ролей, которые этот персонаж не выполняет. Это одно дополнение предотвращает большинство проблем с дрейфом роли.

---

### Шаг 4: Настройте Universal Core

Universal Core — это общий файл, который входит в каждый персонаж вашего офиса. Он содержит правила и протоколы, применимые везде.

1. Скопируйте `templates/TEMPLATE_CORE.md`
2. Задайте **язык** (язык, на котором общаются все персонажи)
3. Определите **стиль общения** (тон, предпочтения формата, настройки длины ответов по умолчанию)
4. Настройте **протоколы качества** (шаги самопроверки, которые персонаж выполняет перед ответом)
5. Добавьте любые **общеорганизационные ограничения**, применимые ко всем ролям

> Если вы собираете только одного персонажа, Universal Core всё равно важен — он обеспечивает чёткое разделение между специфическими для роли правилами (инструкции) и универсальными правилами (Core).
...
2. Назовите проект по роли персонажа (например, "Copywriter", "Analyst")
...
Выполните каждую из этих проверок, прежде чем считать персонажа готовым к продакшену:

| Тест | Что проверяем | Как исправить при сбое |
|---|---|---|
| **Тест роутера** | Находит ли персонаж и загружает ли правильную секцию KB? | Добавьте или уточните ключевые слова-триггеры в роутере |
| **Тест ограничений** | Выполняет ли персонаж каждое железное правило? | Перепишите расплывчатые правила, сделав их проверяемыми |
| **Тест границ** | Остаётся ли персонаж в области действия и отклоняет ли запросы вне неё? | Усильте секцию "Кем я НЕ являюсь" и секцию области действия |
| **Тест делегирования** | Корректно ли персонаж передаёт задачи другим ролям? | Добавьте или исправьте триггеры передачи и формат REQUEST |
...
## 3. Масштабирование до нескольких персонажей (ИИ-офис)

Создание ИИ-офиса — аддитивный процесс. Не пытайтесь проектировать всю систему заранее.

**Последовательность, которая работает:**

1. **Соберите одного персонажа.** Добейтесь хорошей работы. Запустите. Используйте.
2. **Добавьте второго персонажа** с чётко определённой границей от первого. Задокументируйте, какие запросы куда идут.
3. **Создайте Universal Core** — извлеките правила, применимые к обоим персонажам, централизуйте их. Оба персонажа теперь используют этот файл.
...
5. **Создайте протоколы передачи** — определите формат REQUEST, который персонажи используют при делегировании задач друг другу. Единый формат предотвращает сбои при передаче.
6. **Добавляйте персонажей по одному.** Тестируйте каждый новый путь передачи перед добавлением следующего персонажа.
...
Вы пишете 2000 слов контента KB до того, как решили, какова главная директива персонажа. Контент получается разрозненным, и роутер не может его организовать.
...
Вы пытаетесь охватить всё, косвенно связанное с ролью. Персонаж становится универсалом с поверхностными знаниями.
...
**3. Нет роутера**
Персонаж загружает всю KB при каждом запросе. Расход токенов высок, ответы медленные, релевантность снижается.
...
Без явных исключений персонаж берётся за задачи вне своей области — сначала редко, потом систематически.
...
Персонаж выдаёт результат без проверки его на соответствие критериям качества.
...
**7. Слишком много персонажей слишком быстро**
Вы проектируете 6 персонажей до того, как первый заработал. Передачи ломаются, роли перекрываются, отладка превращается в кошмар.
Исправление: один персонаж, полностью рабочий, в реальном использовании. Затем второй. Затем третий. Совокупная ценность офиса появляется только тогда, когда каждый компонент надёжен.
...
**Предупреждение:** Не добавляйте Продюсера до запуска нескольких персонажей. Координатор без объектов координации — впустую потраченные усилия.
...
|---|---|---|
| Собрать одного персонажа | Шаги 1-6 в Секции 2 | `TEMPLATE_KB.md` + `TEMPLATE_INSTRUCTIONS.md` |
| Собрать полный ИИ-офис | Секция 3 (последовательность масштабирования) | Все шаблоны + `TEMPLATE_CORE.md` |
| Добавить специализацию существующему персонажу | Файл аддона для этой специализации | `TEMPLATE_ADDON.md` |
| Масштабировать большую KB (20+ секций) | Иерархический роутер | `TEMPLATE_ROUTER.json` |
| Отладить дрейфующего персонажа | Тесты ограничений и границ (Секция 2, Шаг 6) | Проверьте железные правила в `TEMPLATE_INSTRUCTIONS.md` |
| Настроить общие правила для всех персонажей | Universal Core (Секция 2, Шаг 4) | `TEMPLATE_CORE.md` |
